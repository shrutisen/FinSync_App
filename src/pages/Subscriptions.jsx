...
const Subscription = () => {
  ...
  const [newSubscription, setNewSubscription] = useState({
    ...
    issueDate: "",
    expiry: "",
    isPaid: false, // Initialize new subscriptions as unpaid
  });
  ...
  const addSubscription = () => {
    setSubscriptions([
      ...subscriptions,
      { ...newSubscription, id: subscriptions.length },
    ]);
    resetForm();
  };
  ...
  const handlePayment = (id) => {
    ...
    alert(`Subscription ${id} marked as paid.`);
  };

  const resetForm = () => {
    setNewSubscription({ plan: "", price: "", issueDate: "", expiry: "", isPaid: false });
    setIsEditing(false);
    setEditingId(null);
    setShowForm(false);
  };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <span className="material-icons mr-2">add</span> Add Subscription
        </button>
      </div>
        ...
        {subscriptions.map((subscription) => (
          ...
          style={{
            ...
            className={`bg-white rounded-lg shadow-lg p-4 ${
              subscription.id % 2 === 0 ? "border-l-4 border-purple-400" : ""
            }`}
          >
            ...
            <p className="text-gray-1000 mt-1">{subscription.price}</p>
            <p className="text-gray-1000 text-sm mt-4">Date Issue</p>
            <p className="text-sm font-medium">{subscription.issueDate}</p>
            <p className="text-gray-1000 text-sm mt-4">Expiring</p>
            <p
              className={`text-sm font-medium ${
                subscription.id % 2 === 0 ? "text-red-500" : "text-gray-700"
              }`}
            >
              {subscription.expiry}
            </p>
            ...
            >
              {subscription.isPaid ? "Paid" : "Pay Now"}
            </button>
          </div>
          </div>
        ))}
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <div className="mb-4">
              ...
              <input
                type="date"
                name="issueDate"
                value={newSubscription.issueDate}
                onChange={handleInputChange}
                className="w-full p-2 border rounded"
              />
            </div>
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button
                onClick={isEditing ? updateSubscription : addSubscription}
